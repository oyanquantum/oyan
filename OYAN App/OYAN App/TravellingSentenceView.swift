//
//  TravellingSentenceView.swift
//  OYAN App
//
//  Travelling path – Q3: "Flight was delayed." – put sentence parts in order.
//

import SwiftUI

struct TravellingSentenceView: View {
    let selectedLanguage: Language

    @State private var slots: [String?] = Array(repeating: nil, count: 4)
    @State private var poolParts: [String] = ["қалдырылды", "бүгін", "ұшақ", "кеше", "кейінге", "рейсі", "қонды", "ұшу"].shuffled()
    ...
    // Ұшу рейсі кейінге қалдырылды (Flight was delayed)
    let correctOrder = ["ұшу", "рейсі", "кейінге", "қалдырылды"]

    var questionText: String {
        selectedLanguage == .english ? "Flight was delayed." : "Рейс задержался."
    }
        ...
        VStack(spacing: 0) {
            ...
            VStack(spacing: 20) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    TravellingFlowLayout(spacing: 10) {
                        ForEach(Array(poolParts.enumerated()), id: \.offset) { index, part in
                            TravellingPoolPartChip(part: part, accentColor: buttonColor) {
                                if showFeedback { return }
...
private struct TravellingSlotCell: View {
    let part: String?
...
private struct TravellingPoolPartChip: View {
    let part: String
    let accentColor: Color
    let onTap: () -> Void
...
private struct TravellingFlowLayout: Layout {
...
#Preview {
    NavigationStack {
        TravellingSentenceView(selectedLanguage: .english)
    }
}
