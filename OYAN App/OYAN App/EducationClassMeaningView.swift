//
//  EducationClassMeaningView.swift
//  OYAN App
//
//  Education path – Question 7: "Сабаққа дайындалу" – what does this mean?
//

import SwiftUI

struct EducationClassMeaningView: View {
    ...
    let phrase = "Сабаққа дайындалу"
    
    var options: [String] {
        selectedLanguage == .english
            ? ["To attend a class", "To prepare for class", "To skip class", "To come to class"]
            : ["Посещать урок", "Готовиться к уроку", "Пропустить урок", "Прийти на урок"]
    }
    
    var correctAnswer: String {
        selectedLanguage == .english ? "To prepare for class" : "Готовиться к уроку"
    }
    ...
    var body: some View {
        ...
        .navigationDestination(isPresented: $navigateToNext) {
            EducationSentenceBuildView(selectedLanguage: selectedLanguage)
        }
    }
}

private struct EducationMeaningAnswerButton: View {
    let text: String
    let isSelected: Bool
...
#Preview {
    NavigationStack {
        EducationClassMeaningView(selectedLanguage: .english)
    }
}
