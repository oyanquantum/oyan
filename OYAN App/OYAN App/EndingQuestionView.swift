...
struct EndingQuestionView: View {
    ...
    @State private var navigateToMeaning: Bool = false
    
    let backgroundColor = Color(hex: "#fbf5e0")
    let buttonColor = Color(hex: "#ffa812")
    
    let questionStem = "Мен кеше досыммен кездес___"
    let correctAnswer = "тім"
    let options = ["дан", "дым", "тім", "ым"]
    
    var initialBubbleText: String {
        selectedLanguage == .english ? "Choose the right ending" : "Выбери правильное окончание"
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 20) {
                ...
                // Sentence with blank in styled box (same style as first question)
                ZStack {
        ...
        .navigationDestination(isPresented: $navigateToMeaning) {
            MeaningQuestionView(selectedLanguage: selectedLanguage)
        }
    }
}

private struct EndingAnswerButton: View {
    let text: String
    let isSelected: Bool
...
#Preview {
    NavigationStack {
        EndingQuestionView(selectedLanguage: .english)
    }
}
