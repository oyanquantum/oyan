...
//  OYAN App
//
//  Work & Business path – Q1: "How do you say 'Work'?" – put word together from letters.
//

import SwiftUI

struct WorkWordView: View {
    let selectedLanguage: Language

    // 5 blanks for word жұмыс (work)
    @State private var blanks: [String?] = Array(repeating: nil, count: 5)
    @State private var poolLetters: [String] = ["а", "ж", "и", "р", "с", "і", "м", "ұ", "ы"].shuffled()
    ...
    let correctWord = "жұмыс"

    var questionText: String {
        selectedLanguage == .english ? "How do you say \"Work\"?" : "Как сказать «Работа»?"
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 20) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    WorkFlowLayout(spacing: 10) {
                        ForEach(Array(poolLetters.enumerated()), id: \.offset) { index, letter in
                            WorkPoolLetterChip(letter: letter, accentColor: buttonColor) {
                                if showFeedback { return }
            ...
            VStack {
                HStack {
                    ...
                    ZStack(alignment: .topTrailing) {
                        ...
                        .padding(.top, showFeedback && !isCorrect ? -38 : -30)
        ...
        .navigationDestination(isPresented: $navigateToNext) {
            WorkSalaryMeaningView(selectedLanguage: selectedLanguage)
        }
    }
}

private struct WorkBlankLetterCell: View {
    let letter: String?
...
private struct WorkPoolLetterChip: View {
    let letter: String
    let accentColor: Color
    let onTap: () -> Void
...
private struct WorkFlowLayout: Layout {
...
#Preview {
    NavigationStack {
        WorkWordView(selectedLanguage: .english)
    }
}
