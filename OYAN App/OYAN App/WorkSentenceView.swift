//
//  WorkSentenceView.swift
//  OYAN App
//
//  Work & Business path – Q3: "Company profit increased." – put sentence parts in order.
//

import SwiftUI

struct WorkSentenceView: View {
    let selectedLanguage: Language

    @State private var slots: [String?] = Array(repeating: nil, count: 3)
    @State private var poolParts: [String] = ["компания", "артты", "жақсы", "қаржы", "табысы", "алады", "пайда"].shuffled()
    ...
    // Компания табысы артты (Company profit increased)
    let correctOrder = ["компания", "табысы", "артты"]

    var questionText: String {
        selectedLanguage == .english ? "Company profit increased." : "Прибыль компании выросла."
    }
    ...
    var userOrderCorrect: Bool {
        ...
        return (0..<3).allSatisfy { slots[$0] == correctOrder[$0] }
    }
        ...
        VStack(spacing: 0) {
            ...
            VStack(spacing: 20) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    WorkSentenceFlowLayout(spacing: 10) {
                        ForEach(Array(poolParts.enumerated()), id: \.offset) { index, part in
                            WorkPoolPartChip(part: part, accentColor: buttonColor) {
                                if showFeedback { return }
...
private struct WorkSlotCell: View {
    let part: String?
...
private struct WorkPoolPartChip: View {
    let part: String
    let accentColor: Color
    let onTap: () -> Void
...
private struct WorkSentenceFlowLayout: Layout {
...
#Preview {
    NavigationStack {
        WorkSentenceView(selectedLanguage: .english)
    }
}
