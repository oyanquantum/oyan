//
//  EducationSentenceBuildView.swift
//  OYAN App
//
//  Education path – Question 8: "He prepared carefully for the exam" – put sentence parts in order.
//

import SwiftUI

struct EducationSentenceBuildView: View {
    let selectedLanguage: Language

    // 4 slots for sentence: Ол емтиханға тыңғылықты дайындалды (He prepared carefully for the exam)
    @State private var slots: [String?] = Array(repeating: nil, count: 4)
    // Pool: 4 correct + 4 distractors (кеше, жақсы, тез, жұмыс)
    @State private var poolParts: [String] = ["ол", "кеше", "жақсы", "емтиханға", "тыңғылықты", "тез", "дайындалды", "жұмыс"].shuffled()
    ...
    let correctOrder = ["ол", "емтиханға", "тыңғылықты", "дайындалды"]

    var questionText: String {
        selectedLanguage == .english ? "He prepared carefully for the exam" : "Он тщательно готовился к экзамену"
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 20) {
                ...
                // 4 slots: tap part → first blank; tap filled slot → return to pool
                HStack(spacing: 8) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    SentenceFlowLayout(spacing: 10) {
                        ForEach(Array(poolParts.enumerated()), id: \.offset) { index, part in
                            PoolPartChip(part: part, accentColor: buttonColor) {
                                if showFeedback { return }
...
// MARK: - Slot cell (empty or one part; tap to return to pool)
private struct SentenceSlotCell: View {
    let part: String?
...
// MARK: - Part chip in pool (tap to send to first blank)
private struct PoolPartChip: View {
    let part: String
    let accentColor: Color
    let onTap: () -> Void
...
// Flow layout for wrapping chips (same as Teacher question)
...
#Preview {
    NavigationStack {
        EducationSentenceBuildView(selectedLanguage: .english)
    }
}
